...
*/
void draw_bitmap(Xuint32 BaseAddress, Xuint8 x_crdnt,Xuint8 y_crdnt, Xuint32 bitmap[ROW][COL], char w)
											//ovo je jot    //ovo je i

{
	...
	for(index = 0; index < 25; index++)
		for (j = index + 2 + 32*y_crdnt; j < index + 3 + 32*y_crdnt; j++)
		{
			...
			i = j*(640/8) + k;
			if(w == 'b') PUT_TO_FSL(i, bitmap[index][0]);
			if(w == 'p'){
				PUT_TO_FSL((i-1), bitmap[index][0]);
...
void draw_bitmap_vertical(Xuint32 BaseAddress, Xuint8 x_crdnt,Xuint8 y_crdnt, Xuint32 bitmap[ROW][COL], char c)
{
	int i, j, k, index;
	k = x_crdnt;
	i = BaseAddress;
	for(index = 0; index < 25; index++)
		...
		{
			i = j*(640/8) + k;
			if(c == 'l') PUT_TO_FSL(i, bitmap[index][2]);
			if(c == 'r') PUT_TO_FSL(i, bitmap[index][0]);
		}
}
